...
class HumanTest {
    ...
    void testToString() {
        ...
        String expected = "Human{name='Rain', surname='Josh', year=1999, age=21, iq=99, schedule={Monday=go to Club, Thursday=just sleep, Friday=prepare for the party, Sunday=go to gym, Wednesday=code smth, Tuesday=read a book, Saturday=do whatever should be done}}";
        Human man = new Human("Rain","Josh",1999,(2020 - 1999),99, manSchedule);
        assertEquals(expected.length(),man.toString().length(),"To string not working properly");
    }
    @Test
    void equalsTest(){
